...
#ifndef MARKDOWN_EXAMPLE_SKITY_DEMO_SKITY_DEMO_FONT_MANAGER_H_
#define MARKDOWN_EXAMPLE_SKITY_DEMO_SKITY_DEMO_FONT_MANAGER_H_
...
class SkityDemoFontManager final : public tttext::IFontManager {
 public:
  explicit SkityDemoFontManager(std::filesystem::path font_root) {
    const std::vector<std::pair<tttext::FontDescriptor, std::filesystem::path>>
        fonts = {
            {tttext::FontDescriptor{
                 {"sans-serif"}, tttext::FontStyle::Normal(), 0},
            ...
            {tttext::FontDescriptor{
                 {"NotoSansCJK"}, tttext::FontStyle::Normal(), 0},
             font_root / "Noto_Sans_CJK/NotoSansCJK-Regular.ttc"},
            {tttext::FontDescriptor{
                 {"NotoSansSC"}, tttext::FontStyle::Normal(), 0},
            ...
            {tttext::FontDescriptor{
                 {"NotoColorEmoji"}, tttext::FontStyle::Normal(), 0},
             font_root / "Noto_Color_Emoji/NotoColorEmoji-Regular.ttf"},
            {tttext::FontDescriptor{
                 {"NotoKufiArabic"}, tttext::FontStyle::Normal(), 0},
             font_root /
                 "Noto_Kufi_Arabic/NotoKufiArabic-VariableFont_wght.ttf"},
            {tttext::FontDescriptor{
                 {"NotoSansHebrew"}, tttext::FontStyle::Normal(), 0},
             font_root / "Noto_Sans_Hebrew/"
                         "NotoSansHebrew-VariableFont_wdth,wght.ttf"},
            {tttext::FontDescriptor{
                 {"NotoSansThai"}, tttext::FontStyle::Normal(), 0},
             font_root /
                 "Noto_Sans_Thai/NotoSansThai-VariableFont_wdth,wght.ttf"},
        };

    for (const auto& item : fonts) {
      const auto path = item.second.string();
      auto typeface = MakeTypeface(path.c_str(), 0);
      if (typeface != nullptr) {
  ...
  ::TypefaceRef matchFamilyStyleCharacter(const char family_name[],
                                          const tttext::FontStyle& style,
  ...
  ::TypefaceRef legacyMakeTypeface(const char family_name[],
                                   tttext::FontStyle style) const override {
    (void)style;
...
#endif  // MARKDOWN_EXAMPLE_SKITY_DEMO_SKITY_DEMO_FONT_MANAGER_H_

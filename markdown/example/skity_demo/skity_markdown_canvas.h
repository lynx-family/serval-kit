...
#ifndef MARKDOWN_EXAMPLE_SKITY_DEMO_SKITY_MARKDOWN_CANVAS_H_
#define MARKDOWN_EXAMPLE_SKITY_DEMO_SKITY_MARKDOWN_CANVAS_H_
...
class SkityMarkdownCanvas final : public tttext::SkityCanvasHelper,
 ...
 private:
    ...
    for (const auto& op : path->path_ops_) {
      switch (op.op_) {
        ...
        case MarkdownPath::PathOpType::kQuadTo:
          result.QuadTo(op.data_.quad_.control_.x_, op.data_.quad_.control_.y_,
                        op.data_.quad_.end_.x_, op.data_.quad_.end_.y_);
          break;
        case MarkdownPath::PathOpType::kCubicTo:
          result.CubicTo(
              op.data_.cubic_.control_1_.x_, op.data_.cubic_.control_1_.y_,
              op.data_.cubic_.control_2_.x_, op.data_.cubic_.control_2_.y_,
  ...
  skity::Paint MakeGradientPaint(MarkdownLinearGradient* gradient,
    ...
    const float* stops = gradient->stops.size() == gradient->colors.size()
                             ? gradient->stops.data()
                             : nullptr;
    paint.SetShader(skity::Shader::MakeLinear(points, colors.data(), stops,
...
#endif  // MARKDOWN_EXAMPLE_SKITY_DEMO_SKITY_MARKDOWN_CANVAS_H_

...
lynx::markdown::SizeF AndroidMarkdownView::Measure(lynx::markdown::MeasureSpec spec) {
  auto* env = MarkdownClassCache::GetEnv();
  jlong result = env->CallLongMethod(ref_.Get(), methods_.measure_, spec.width_,
...
lynx::markdown::SizeF AndroidCustomView::Measure(lynx::markdown::MeasureSpec spec) {
...
void AndroidMainView::Initialize(JNIEnv* env) {
  ...
  methods_.create_custom_subview_ = env->GetMethodID(
      clazz, "createCustomDrawView", "()Lcom/lynx/markdown/MarkdownViewHandle;");

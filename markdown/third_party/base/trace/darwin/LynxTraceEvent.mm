...
+ (void)beginSection:(NSString*)category
  ...
  TRACE_EVENT_BEGIN(
      [category UTF8String], nullptr, [&](lynx::perfetto::EventContext ctx) {
        auto event = ctx.event();
        event->set_name([name UTF8String]);
        [keyValues enumerateKeysAndObjectsUsingBlock:^(
                       id _Nonnull key, id _Nonnull obj, BOOL* _Nonnull stop) {
          auto* debug = event->add_debug_annotations();
          debug->set_name([[NSString stringWithFormat:@"%@", key] UTF8String]);
          debug->set_string_value(
              [[NSString stringWithFormat:@"%@", obj] UTF8String]);
        }];
      });
}

+ (void)beginSection:(NSString*)category withName:(NSString*)name {
  TRACE_EVENT_BEGIN([category UTF8String], nullptr,
...
+ (void)instant:(NSString*)category
  ...
  TRACE_EVENT_INSTANT([category UTF8String], nullptr,
                      [&](lynx::perfetto::EventContext ctx) {
                        ctx.event()->set_name([name UTF8String]);
...
+ (void)counter:(NSString*)category
            withName:(NSString*)name
    withCounterValue:(uint64_t)value {
  TRACE_COUNTER([category UTF8String],
                lynx::perfetto::CounterTrack([name UTF8String]), value);
}

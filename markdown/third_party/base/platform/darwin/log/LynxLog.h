...
#define LErrWarn(expression, errCode, ...) \
  _LynxErrorWarning(expression, errCode, __VA_ARGS__)
#define LErrFatal(expression, errCode, ...) \
  _LynxErrorFatal(expression, errCode, __VA_ARGS__)
...
@property(nonatomic, assign) NSInteger
    acceptRuntimeId;  // -1 means receiving all runtime logs, default is -1
...
LYNX_BASE_EXTERN void _LynxLogInternal(const char*, int32_t, LynxLogLevel,
                                       NSString*, ...) NS_FORMAT_FUNCTION(4, 5);
LYNX_BASE_EXTERN const char* _GetLastPath(const char* filename, int32_t length);

#define _LynxErrorInfo(errCode, ...) \
  _LynxErrorInfoInternal(errCode, __VA_ARGS__)
